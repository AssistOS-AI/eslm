...
const STRATEGY_FAMILIES = Object.freeze([
  ...
  ['family:strategy:task-context', strategyIds((item) => item.stage === 'runtime.context.construct')],
...
const RUNTIME_NODES = [
  ...
  node('node:runtime:task-context-coordinator', 'Task context coordinator',
    'circuit:runtime:knowledge-routing', 'coordinator', 'runtime.context.construct',
    'Recognizes explicit and embedded basic questions, expands bounded self-questions over request topics, and retrieves a query-local non-authoritative context frontier.', {
      inputs: [P('runtime:language-assessment')], outputs: [P('runtime:task-knowledge-context')],
      owner: 'src/runtime/task-context-construction.mjs',
      strategies: ['strategy:context:question-facet-expansion@1'],
      families: ['family:strategy:task-context'],
      resources: [R('tokens'), R('candidates'), R('lookups'), R('evidence-items'), R('output-bytes')],
    }),
  node('node:runtime:request-operation-framer', 'Request operation framer',
    'circuit:runtime:request-session', 'process', 'runtime.request.plan',
    'Represents one admitted bounded operation, its request-supplied operands, and its output obligation without executing it.', {
      inputs: [P('runtime:direct-diagnostic')], outputs: [P('runtime:bounded-operation-frame')],
      owner: 'src/language/bounded-operation-framing.mjs', resources: [R('tokens'), R('graph-nodes')],
    }),
  node('node:runtime:deterministic-value-executor', 'Deterministic value executor',
    'circuit:runtime:method-selection', 'process', 'runtime.reason.execute',
    'Executes declared arithmetic, percentage, unit, clock, sequence, grouping, remainder, mean, and strict-order operations with a replayable witness.', {
      inputs: [P('runtime:bounded-operation-frame')], outputs: [P('runtime:deterministic-value-result')],
      owner: 'src/reasoning/deterministic-value-operations.mjs',
  ...
  node('node:runtime:supplied-text-operator', 'Supplied-text operator',
    ...
    'Classifies, extracts, corrects, or rewrites only the bounded text supplied in the request and records its supporting spans or cues.', {
      inputs: [P('runtime:bounded-operation-frame')], outputs: [P('runtime:supplied-text-result')],
      owner: 'src/reasoning/supplied-text-operations.mjs',
  ...
  node('node:runtime:grounded-knowledge-inspector', 'Grounded knowledge inspector',
    ...
    'Resolves a requested entity or class against loaded declarative KB facts and realizes only the facts actually found.', {
      inputs: [P('runtime:bounded-operation-frame')], outputs: [P('runtime:knowledge-inspection-result')],
      owner: 'src/reasoning/grounded-knowledge-inspection.mjs',
      resources: [R('facts'), R('lookups'), R('output-bytes')],
    }),
  node('node:runtime:typed-operation-result-assembler', 'Typed operation result assembler',
    ...
    'Builds a query-local runtime candidate with the exact frame, method, witness, provenance, and unresolved gap.', {
      inputs: [P('runtime:deterministic-value-result'), P('runtime:knowledge-inspection-result'),
        P('runtime:supplied-text-result')], outputs: [P('runtime:construction-candidate')],
      owner: 'src/runtime/bounded-operation-processing.mjs', resources: [R('output-bytes'), R('proof-bytes')],
    }),
...
const RUNTIME_EDGES = [
  ...
  edge('edge:runtime:language-task-context', 'node:runtime:english-likelihood-gate', 'node:runtime:task-context-coordinator', 'data', P('runtime:language-assessment'), 'likely-english-or-indeterminate'),
  ...
  edge('edge:runtime:parser-operation-frame', 'node:runtime:direct-parser-gate', 'node:runtime:request-operation-framer', 'data', P('runtime:direct-diagnostic'), 'direct-answer-not-complete'),
  edge('edge:runtime:operation-frame-deterministic', 'node:runtime:request-operation-framer', 'node:runtime:deterministic-value-executor', 'control', P('runtime:bounded-operation-frame'), 'deterministic-value-or-order-operation'),
  edge('edge:runtime:operation-frame-supplied-text', 'node:runtime:request-operation-framer', 'node:runtime:supplied-text-operator', 'control', P('runtime:bounded-operation-frame'), 'supplied-text-operation'),
  edge('edge:runtime:operation-frame-knowledge', 'node:runtime:request-operation-framer', 'node:runtime:grounded-knowledge-inspector', 'control', P('runtime:bounded-operation-frame'), 'knowledge-inspection-operation'),
  edge('edge:runtime:operation-frame-bypass', 'node:runtime:request-operation-framer', 'node:runtime:request-force-gate', 'control', P('runtime:direct-diagnostic'), 'no-supported-bounded-operation'),
  edge('edge:runtime:deterministic-operation-result', 'node:runtime:deterministic-value-executor', 'node:runtime:typed-operation-result-assembler', 'data', P('runtime:deterministic-value-result'), 'operation-complete'),
  edge('edge:runtime:supplied-text-operation-result', 'node:runtime:supplied-text-operator', 'node:runtime:typed-operation-result-assembler', 'data', P('runtime:supplied-text-result'), 'operation-complete'),
  edge('edge:runtime:knowledge-inspection-result', 'node:runtime:grounded-knowledge-inspector', 'node:runtime:typed-operation-result-assembler', 'data', P('runtime:knowledge-inspection-result'), 'inspection-complete-or-explicit-gap'),
  edge('edge:runtime:typed-operation-assemble-schema', 'node:runtime:typed-operation-result-assembler', 'node:runtime:result-schema-gate', 'data', P('runtime:construction-candidate'), 'candidate-constructed'),
  ...
  edge('edge:runtime:context-focus', 'node:runtime:task-context-coordinator', 'node:runtime:knowledge-focus-coordinator', 'data', P('runtime:task-knowledge-context'), 'context-focus-available'),
  ...
  edge('edge:runtime:context-construct', 'node:runtime:task-context-coordinator', 'node:runtime:result-construction-coordinator', 'data', P('runtime:task-knowledge-context'), 'context-available-for-answer-or-explicit-gap'),
