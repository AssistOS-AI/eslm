...
This repository develops an Executable Symbolic Language Model (ESLM): a deterministic symbolic runtime, a declarative knowledge-package format, and an offline toolchain that compiles evidence into reviewable data. Training-time coding agents may analyze prepared training evidence and propose declarative records. The deployed runtime never calls an LLM or agent and never executes knowledge-base content as source code. Its language boundary is English-only: a bounded heuristic likelihood gate may reject likely non-English input, but it never translates or claims to understand it. The general CLI uses the fully local symbolic profile by default. `--external-language-agent` or interactive `/normalize on` explicitly composes and discloses the separate DS013 Language Agent wrapper; `--no-external-language-agent` or `/normalize off` explicitly retains the local default. The wrapper may propose translation for a likely-non-English rejection or conservative English simplification after local English routes remain `UNPARSED`. A Language Agent may be a general coding agent or a translation-focused model; the currently supported adapter invokes Codex. It is one untrusted proposal strategy at the language-interpretation node, not part of deployed symbolic inference, and every candidate returns through the unchanged non-voting English parser and semantic gate. It has no answer, KB, proof, or result authority.
...
24. For natural basic-question recognition, mandatory early KB context, self-question expansion, or cited contextual fallback, read `docs/specs/DS035-query-local-epistemic-context-construction.md` completely and keep contextual relevance separate from premise, interpretation, proof, and strict-answer authority.
...
- The general CLI and deployed/library runtime use the fully local symbolic profile by default. A bounded English-likelihood gate first rejects likely non-English input locally without translating it. For likely-English and indeterminate input, the mandatory DS035 context strategy recognizes explicit and embedded basic questions and retrieves a bounded non-authoritative KB frontier before direct execution; that context never enters language proposal selection. The direct route, bounded request planning, and eligible DS022 approximation then run. Every changed interpretation remains query-local and a strict answer through it is reported as `DEFEASIBLE`; grounded symbolic request construction and cited contextual fallback are `PARTIAL`. Grounding exposure or context realization occurs only after the final route is known.
- `--external-language-agent` or interactive `/normalize on` explicitly opts into the disclosed assisted profile; `--no-external-language-agent` and `/normalize off` explicitly retain the local default. A local `UNPARSED` Thinking summary may recommend that opt-in, but recommendation never invokes an external process. When opted in, a likely-non-English rejection may request translation, while English simplification is eligible only after local recovery remains `UNPARSED`. One assisted episode permits at most three language proposals and may return only parser-form feedback, never answers, KB evidence, grounding, or proof state. Every candidate is assessed as English and reparsed by the direct symbolic runtime. Translation remains `UNVERIFIED_NORMALIZATION` unless an independent reviewed language profile establishes open-class preservation; model-declared alignments are not self-validating.
- Every ordinary question emits one subtle bounded-symbolic start line with its effective profile and decisive caps.
  Immediately before each real external Language Agent call it additionally emits exactly one attempt-aware line
  naming operation, proposal slot, adapter, and per-call timeout. `ask` and `run` write activity to standard error so
  JSON or JSONL standard output remains valid. Direct execution, the fully local profile, and cache hits emit no
  external-invocation line. Every completed interactive result retains the gray operational summary; contained
  per-request validation failures leave the prior session state intact and return to the prompt.
- `balanced` is the default `eslm-work-policy-v1` profile. `quick`, `deep`, and `exhaustive-bounded` select other finite limits for heuristic candidates and reparses, Horn work, provider fan-out, DS035 context, and grounding retrieval. CLI startup flags and interactive `/work PROFILE` may change these budgets without changing logic, trust, selected KB identities, or session facts.
...
- Results use `eslm-runtime-result-v1`. Every text result exposes protocol, status, answer, session and episode state,
  ...
  may be absent after an early `UNPARSED`. Eligible English results may expose an
  `eslm-task-knowledge-context-v1` extension with 35-family question analysis, bounded self-questions, focus,
  provenance-bearing entries, search completeness, and no answer, premise, or interpretation authority. Eligible
  inability results may also expose an
...
- Basic-question taxonomy and query-local context: `src/language/basic-question-taxonomy.mjs` and `src/runtime/task-context-construction.mjs`
...
- Query-local epistemic context and natural basic-question contract: `docs/specs/DS035-query-local-epistemic-context-construction.md`
