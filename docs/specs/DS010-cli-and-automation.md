...
### Question #1: Why does `ask` return JSON instead of only a sentence?
...
### Question #2: Should the CLI call external model APIs for comparison?
...
### Question #3: Should batch input accept a single full document?
...
### Question #4: Why are tutorial examples tied to the promoted model?
...
### Question #5: Why must `--kb` be explicit rather than stored as a global default?
...
### Question #6: Why does `/examples` show a representative sample instead of the complete catalog?

Response: The command is a discovery interface, not a test log. Dumping every generated case buried the capability boundary under renamed repetitions. It now shows up to four seeded representatives from every group, while `/smoke` executes a smaller sample and the complete 204-case and 1,000-case gates remain available through tests.

### Question #7: Why can `/memory` report lazy loading without a memory number?
...
### Question #8: Why does `/smoke` print a seed?

Response: The default should vary examples between invocations, but a failure must be exactly reproducible. Passing the printed seed to `/smoke SEED` or `/examples SEED` reconstructs the same selection without introducing hidden example IDs.
