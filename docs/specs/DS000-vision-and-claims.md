...
The first completed public-data run is bAbI v1.2 Task 15 English 10k. The coding agent analyzed 10,000 train episodes in 20 hashed chunks. The promoted artifact retains three constructions and four morphology mappings but zero episode entities, facts, rules, or answers. The untouched prepared test split contains 1,000 episodes and the default artifact currently produces 1,000 correct semantic values. This evidence supports only the controlled one-step class-deduction capability measured by Task 15.
