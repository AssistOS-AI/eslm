...
K1 source compilation is complete: WordNet has a frozen manifest, complete generated shards, eager and bounded lazy query integration, validation, inventories, ambiguity-preserving lookup, hypernym tests, randomized checks, and performance measurements. K1 remains open on contextual word-sense disambiguation and compact postings. K2 source compilation and relation-specific realization are complete for ATOMIC train data, including eager and bounded lazy lookup; K2 remains open on general participant binding, calibrated ranking, generated token indexing, and dev/test evaluation. K3 applies the source-build standard to ConceptNet only after its own probe and compiler gates close. K4 is bounded GeoNames. K5 remains optional thematic Wikidata.
