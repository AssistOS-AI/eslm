...
Induction runs only for queries explicitly compiled with `reasoning: induction` or for a model-declared implicit property question such as Task 16 color. It considers only allowlisted predicates, groups observations by class and predicate/value, applies per-predicate support, coverage, and selection policy, and emits candidate facts tagged `reasoning: induction`, confidence, support count, population count, conflict count, and complete source evidence. A factual `Can X VERB?` query never consumes these candidates; `Is X likely to VERB?` and configured `What color is X?` queries may return `INDUCTIVE`.
